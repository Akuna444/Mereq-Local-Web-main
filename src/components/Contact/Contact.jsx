"use client";
import { useState } from "react";
import Reveal from "../UI/Animations/Reveal";

const Contact = () => {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [phone, setPhone] = useState("");
  return (
    <div className="section-padding">
      <div className="w-full flex flex-col items-center text-center">
        <Reveal>
          <h2 className="dark:text-secondary text-primary mb-5 font-bold">
            Start with Free marketing consultation
          </h2>
        </Reveal>

        <h4 className="dark:text-white  text-black w-[80%] md:w-[60%]">
          <Reveal>
            {" "}
            Fill out the following form below and we will call you to schedule a
            consultation
          </Reveal>
        </h4>

        <form className="flex flex-col w-[90%] md:w-[50%]  gap-2">
          <div className="flex flex-col">
            <label className="text-left my-2 text-primary dark:text-tertiary">
              <h6>Name</h6>
            </label>
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              type="text"
              className="contact-input"
            />
          </div>

          <div className="flex flex-col">
            <label className="text-left my-2 text-primary dark:text-tertiary">
              <h6>Phone Number</h6>
            </label>
            <input
              value={phone}
              onChange={(e) => setPhone(e.target.value)}
              type="text"
              className="contact-input"
            />
          </div>
          <div className="flex flex-col">
            <label className="text-left my-2 text-primary dark:text-tertiary">
              <h6>Email</h6>
            </label>
            <input
              onChange={(e) => setEmail(e.target.value)}
              value={email}
              type="email"
              className="contact-input"
            />
          </div>
          <div className="flex my-8 text-white w-full justify-center">
            <Reveal>
              <button
                type="submit"
                className="dark:bg-[#146154] bg-secondary bg-opacity-70 dark:bg-opacity-100 hover:dark:bg-[#146154]/80 hover:bg-secondary/80 shadow-glow border-gray-300 border-2 rounded-[20px] p-1 w-[80%] md:w-fit   md:p-5"
              >
                <h5 className="font-semibold">Request FREE Consultation</h5>
              </button>
            </Reveal>
          </div>
          <div className="dark:text-white text-primary">
            <Reveal>
              <p>We will use your information to contact you</p>
            </Reveal>
            <Reveal>
              <p>
                Read more about how we use private data in
                <a href="#" className="text-tertiary">
                  &nbsp;Privacy and Policy
                </a>
              </p>{" "}
            </Reveal>
          </div>
        </form>
      </div>
    </div>
  );
};

export default Contact;
